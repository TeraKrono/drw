...
const words = [
    ...
    'килим', 'кіно', 'кисень', 'ключ', 'ковдра', 'коло', 'конверт', 'корабель',
    ...
    'миргородська', 'стрім', 'тікток', 'пілоти', 'живчик'
];
...
io.on('connection', (socket) => {
    ...
    socket.on('clear-canvas', (data) => {
        ...
        io.to(socket.roomId).emit('clear-canvas', data);
    });
    ...
    socket.on('fill-background', (data) => {
        ...
        socket.to(socket.roomId).emit('fill-background', data);
    });
